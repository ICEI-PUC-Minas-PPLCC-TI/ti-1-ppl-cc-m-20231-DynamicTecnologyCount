...
var espacos_hospitais = [
  {local: 1, pessoas_max: 500, funcionarios_min: 150, presentes_local_enfermeiro: 90, presentes_local_tecnico: 40},
  {local: 2, pessoas_max: 300, funcionarios_min: 100, presentes_local_enfermeiro: 100, presentes_local_tecnico: 30},
  {local: 3, pessoas_max: 100, funcionarios_min: 50, presentes_local_enfermeiro: 45, presentes_local_tecnico: 10},
]

let precisa = 0;

for(let i = 0; i < espacos_hospitais.length; i++){
  
  let soma = 0;

  soma += espacos_hospitais[i].presentes_local_enfermeiro + espacos_hospitais[i].presentes_local_tecnico;


  if(soma < espacos_hospitais[i].funcionarios_min){

    precisa += espacos_hospitais[i].funcionarios_min - soma;

  }

}

let enfermeiro = parseInt(precisa/1.5);
let tecnico = parseInt(precisa/4);

var contratacao = {

  enfermeiros_a_contratar: `${enfermeiro}`,
  tecnicos_a_contratar: `${tecnico}`,

}

var gastos_da_contratacao = (contratacao.enfermeiros_a_contratar * 2000) + (contratacao.tecnicos_a_contratar * 1500);
